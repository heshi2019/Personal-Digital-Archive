...
class GcoresApi:
    ...
    def get_cookie(self):
        ...
        cookie = config.get_key("GRCORES","cookie")
        if cookie is None:
    ...
    def get_Radios(self):
        ...
        config.save("Data_Star", "Gcores_Radios.json", radiosList, "txt")
        config.save("Data_End", "Gcores_Categories.json", categoriesList, "txt")
        config.save("Data_Star", "Gcores_User.json", usersList, "txt")
        config.save("Data_Star", "Gcores_albums.json", albumsList, "txt")
    ...
    def get_saveRadiosData(self,radios,
        ...
        for item in radios:
            ...
            if item.get("type") == "radios":
                ...
                content = ""
                # 节目介绍
                # 注意字典的get方法，只有在key不存在时，才会返回默认值，如果key存在但值为None，不会返回默认值。
