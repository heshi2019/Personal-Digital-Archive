...
class QianJiApi:
    ...
    def get_catefories(self):
        cookie = config.get_key("QIANJI","cookie")
        headers = {
        ...
        if r.ok:
            data = r.json()
            # 筛选数据
            config.save("Data_Star", "qianji_catefories_data.json",data.get("data",""), "txt")
    ...
    def get_data(self):
        ...
        if r.ok:
            data = r.json()
            # 筛选数据
            config.save("Data_Star", "qianji_data.json",data.get("data",""), "txt")
