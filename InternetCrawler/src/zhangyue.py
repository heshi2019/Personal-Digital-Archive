...
def read_books_data():
    ...
    with open('BasicData\\zhangyue\\掌阅-批注书籍.txt', 'r', encoding='utf-8') as file:
        json_data = file.read()
...
def save_json(books_data):
    ...
    config.save("Data_End", "zhangyue.json", final_books_data, "txt")
    print("掌阅数据解析完毕")
...
def main():
    ...
    # 读取批注数据
    annotations = read_annotation_files('BasicData\\zhangyue\\掌阅笔记')
