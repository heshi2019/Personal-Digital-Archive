...
class FlymeApi:
    ...
    def get_cookie(self):
        ...
        cookie = config.get_key("FLYME","cookie")
        if cookie is None:
    ...
    def get_Classification(self):
        ...
        if r.ok and r.json().get("returnCode",302) != 302:
            config.save("Data_Star", "Flyme_Classification.json", r.json().get("returnValue",{}).get("data"), "txt")
        else:
            errcode = r.json()
    ...
    def get_Flyme_data(self):
        ...
        if r.ok and r.json().get("returnValue",{}).get("content",None) is not None:
            config.save("Data_Star", "Flyme_data.json", r.json().get("returnValue",{}).get("content"), "txt")
        else:
            errcode = r.json()
