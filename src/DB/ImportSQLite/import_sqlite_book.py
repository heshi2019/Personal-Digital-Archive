...
class BookRepository:
    ...
    def reader_file(self, file_path: str):
        ...
        # 微信读书
        data_path = os.path.join(app_config.Data_End, file_path)  # 改为平级目录

        files_found = glob.glob(data_path)

        if not files_found:
            print(f"错误：在文件夹 '{app_config.Data_End}' 中没有找到匹配 '{file_path}' 的文件。")
        else:
        ...
        try:
            ...
            for key, value in data.items():
                ...
                insert_data = (
                    ...
                    json.dumps(value["1000000"], ensure_ascii=False),
                    value["cover"],
                    value["name"],
                    value["isbn"],
                    value["readSign"],
