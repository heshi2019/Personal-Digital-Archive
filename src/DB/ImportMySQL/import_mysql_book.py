...
def import_book_Mysql():
    print("开始导入 book 数据 --> mysql")

    # 微信读书
    data_path = os.path.join(app_config.Data_End, 'weread.json')
    parse_and_insert(data_path,"weread")

    # 网易蜗牛
    data_path = os.path.join(app_config.Data_End, 'du.json')
    parse_and_insert(data_path, "du")

    # 掌阅
    data_path = os.path.join(app_config.Data_End, 'zhangyue.json')
