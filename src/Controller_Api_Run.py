...
class ApiRun:
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--scan-only', action='store_true', help='只运行扫描器，不启动 API 服务')
        self.parser.add_argument('--full', action='store_true', help='强制进行完整扫描（重新计算哈希值/缩略图）')
    ...
    def run(self):
        ...
        ''' 
            ...
            客户端需要通过这个端口访问API服务
        '''
        uvicorn.run("src.controller.server:app", host="127.0.0.1", port=5000)
